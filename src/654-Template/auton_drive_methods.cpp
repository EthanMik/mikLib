...
void auton_drive_methods::turn_left_arc(float radius, float arcLength, float speed){ //RADIUS IN INCHES
    if(radius == 0) return;
    ...
    while (false || get_vector_distance(chassis.get_position_vector(), targetDestination) > 2) {
        ...
        rightDrive.spin(vex::fwd, rightSpeed, velocity_units::normalized);
        leftDrive.spin(vex::fwd, leftSpeed, velocity_units::normalized);

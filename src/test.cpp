...
void test_motion_chaining() {
	// Setting default values, can also be done struct params
	constants.drive_min_voltage = 4;
	constants.turn_min_voltage = 6;
...
void config_tune_drive() {
	...
	graph_scr->set_title("Drive PID Output");
	graph_scr->set_plot_bounds(y_min, y_max, 0, time_spent_graphing_ms, 1, 1);
	graph_scr->set_plot({actual_plot, set_point_plot}, {{"Actual", "#2E8B59"}, {"SetPoint", "#FA8072"}});
...
void config_tune_heading() {
	...
	graph_scr->set_title("Heading PID Output");
	graph_scr->set_plot_bounds(y_min, y_max, 0, time_spent_graphing_ms, 1, 1);
	graph_scr->set_plot({[](double x) { return chassis.inertial.rotation(); },
	                     [](double x) {
...
void config_tune_turn() {
	...
	graph_scr->set_title("Turn PID Output");
	graph_scr->set_plot_bounds(y_min, y_max, 0, time_spent_graphing_ms, 1, 1);
	graph_scr->set_plot({[](double x) { return chassis.get_absolute_heading(); },
...
void config_tune_swing() {
	...
	graph_scr->set_title("Swing PID Output");
	graph_scr->set_plot_bounds(y_min, y_max, 0, time_spent_graphing_ms, 1, 1);
	graph_scr->set_plot({[](double x) { return chassis.get_absolute_heading(); },
	...
	update_controller_scr = vex::task([]() {
		while (1) {
			...
			if (btnDown_new_press(Controller.ButtonDown.pressing())) {
				if (data.index < data.variables.size() - 1) {
					data.index++;
				}
				data.modifer_scale = 1;
			}

...
// void standardized_vector_movement(std::vector<float> origin, float speed, std::function<std::vector<float>(float, float, float, float)> vector_field, int optimizationMode, float adjustment_sharpness){
//     float dotProduct;
//     float lengthOfVector;
...
//         newDirection = vector_field(xPos, yPos, adjustment_sharpness);
//         lengthOfVector = get_vector_distance({0.0f, 0.0f}, newDirection);
//         newDirection[0] /= lengthOfVector;
//         newDirection[1] /= lengthOfVector;
//         dotProduct = newDirection[0] * currentDirection[0] + newDirection[1] * currentDirection[1]; //Gives relative closeness to direction (1:same, -1:complete opposites)
...
//             rightSpeed = speed;
//         }
//         optimization(leftSpeed, rightSpeed, xPos, yPos, speed);
//         Controller.Screen.setCursor(1,1);
...
// void optimization(int mode, &x, &y,){}
